...
rain21 = subset(rain1,subset=(year == 2021))

# load Pload
load('Ploads+ortho_PB+YP_2020-2021.Rdata')
print(Pload[1,])
Pload$PBYW = Pload$PB.kg.d + Pload$YW.kg.d

# match the year
PL21 = subset(Pload,subset=(year==2021))
PL21$idoy = PL21$doy

# for these drivers, keep 1 March - 30 Sept, doy 61 - 274
train21 = subset(rain21,subset=(idoy>=61 & idoy<=274))
tPL21 = subset(PL21,subset=(idoy>=61 & idoy<=274))

# merge PL and precip
d1 = as.data.frame(train21)
d2 = as.data.frame(tPL21)
PLppt0 = merge(d1,d2,by=c('year','idoy'))
# sort
PLppt21 = PLppt0[with(PLppt0, order(idoy)), ] # sort
...
print(daily21[1,])
print(range(daily21$idoy),quote=F)
print('doy range of PLoad & ppt',quote=F)
print(range(PLppt21$idoy),quote=F)

# For the drivers + buoy variates use 15 March - 30 Sept,
#  doy 136-274
datall0 = merge(daily21,PLppt21,by=c('year','idoy'))
# check the sort
...
save(all21,file='Buoy+PLppt_daily21.Rdata')